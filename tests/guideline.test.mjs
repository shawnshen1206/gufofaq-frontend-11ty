...
test("§5 元件 js 查詢的 class 選擇器都要在 src markup 打得到（否則是打不到東西的死 js）", () => {
    ...
    const productionHtml = srcHtml.filter((p) => !p.replace(/\\/g, "/").endsWith("pages/components/component.html"));
    const markupClasses = new Set();
    for (const f of productionHtml)
        for (const m of read(f).matchAll(/class="([^"]*)"/g))
            for (const c of m[1].split(/\s+/)) if (c && !c.includes("{")) markupClasses.add(c);
    assert.ok(markupClasses.size > 200, `markup class 只掃到 ${markupClasses.size} 個 —— 這條測試在空轉`);
