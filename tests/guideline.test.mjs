...
test("每個 openModal('X') 在同一頁上都要找得到 <dialog id=\"X\">", () => {
    ...
    for (const f of distHtml) {
        const html = read(`dist/${f}`);
        const ids = new Set([...html.matchAll(/\bid="([^"]+)"/g)].map((m) => m[1]));
        for (const m of html.matchAll(/openModal\(\s*['"]([^'"]+)['"]/g))
            if (!ids.has(m[1])) hits.push(`dist/${f}  openModal("${m[1]}") 在本頁找不到對應的 id`);
    }
...
test("HTML 註解的 <!-- --> 必須配對（斷掉的話註解內文會變成頁面上的可見文字）", () => {
    ...
    const bad = [];
    for (const f of srcHtml) {
        // 先把內嵌 <script> 挖空：JS 字串裡可能出現字面的 "-->"，會被誤判成孤兒
...
test("§4 文字族 token 不可拿去當 background-color / border-color", () => {
    ...
    const TEXT = "text|text-strong|text-muted|brand-text|brand-text-hover|danger-text";
...
const COLOR_ROLES = {
    ...
    textOnSurface: ["--text", "--text-strong", "--text-muted", "--brand-text", "--brand-text-hover", "--danger-text", "--brand-ink"],
...
test("§4 對比度硬規則：逐色實算（白字疊填充 ≥4.5、填充對底色 ≥3、內文疊表面 ≥4.5）", () => {
    ...
    for (const [mode, at] of [["light", /^:root\s*\{/m], ["dark", /^\[data-theme="dark"\]\s*\{/m]]) {
        ...
        for (const c of textOnSurface) for (const bg of ["--surface", "--surface-raised"]) check(ratio(c, bg), 4.5, `內文 ${c} on ${bg}`);
