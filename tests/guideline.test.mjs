...
test("§4 不得依頁面覆寫元件（body-class 範圍選擇器只准出現在該頁自己的 chrome 檔）", () => {
    ...
    for (const f of srcScss) {
        read(f).split("\n").forEach((line, i) => {
            const m = line.match(/\.((?:guideline|catalog|chatbot)-page)\b/);
...
test("§5 hook class 不得被 scss 樣式（.js-* 與具名真 app hook 全站 scss 零命中）", () => {
    ...
    const NAMED_HOOKS = ["copyBtn", "watchBtn", "shareBtn", "btn-prev", "btn-next", "calendar", "singleSelect", "multiSelect"];
...
test("§4 <table> 直下不放 <tr>（一律包 thead/tbody，否則 SSR/hydration 兩邊樹不同）", () => {
    ...
    let tables = 0;
    for (const f of distHtml) {
        for (const m of distDoc(f).matchAll(/<table[^>]*>\s*<(\w+)/g)) {
            tables++;
            if (m[1].toLowerCase() === "tr") hits.push(`dist/${f}  <table> 直下就是 <tr>（包 tbody）`);
        }
    }
...
test("§6 元件內部 {% set %} 示範變數名：跨元件唯一、且不與頁面層變數同名（靜默覆蓋沒有其他測試抓得到）", () => {
    ...
    for (const f of srcHtml.filter((x) => x.includes("_includes"))) {
        for (const m of read(f).matchAll(setName)) {
