...
const COLOR_ROLES = {
    ...
    pairs: [
        ...
        // round15：round14 手修 agent-activity chip（--brand-text 疊 sunken 4.49 < AA → 改 --brand-ink）
        // 卻只把重算數字寫進 scss 註解——沒進 pairs 就能無聲回歸。sunken 面上的真實疊法都要在這裡
        // （新增 sunken 上的字色時記得補列——這份清單靠人手跟 markup，漏了測試就少一組防回歸）。
        ["--brand-ink", "--surface-sunken"], // agent-activity 工具名 chip、chat-message 行內碼
        ["--text", "--surface-sunken"], // code-block／agent-activity 參數碼、chat-message pre
...
test("§5 元件 js 查詢的 class 選擇器都要在 src markup 打得到（否則是打不到東西的死 js）", () => {
    ...
    // component.html 真的存在，否則照樣紅。
    const SHOWCASE_INTERACTION = new Set(["top-tabs", "sub-tabs"]);
...
test("§4-2 繁中原文相同的 chrome 沿用既有 key、不另立（同文異 key 遲早讓英譯自己分岔）", () => {
    ...
    const DELIBERATE = new Set([
        "時間", "標題", "內容", "檔案名稱", "資料集名稱", "資料集",  // dataImport/dataset/audit 各區段表頭語境（round15 裁決暫留的舊家族）
        "開始時間", "結束時間", "關鍵字", "狀態",                          // qa 篩選 vs settings 統計篩選；批次匯入欄 vs widget 欄
        "無", "結果", "共", "讚", "倒讚", "上一頁", "筆",                 // 量詞/前綴/評價的組字上下文各異（前後綴空白規格不同）
        "登入", "至少 8 碼", "刪除", "設定", "所屬群組",                  // 管理端 vs 前台 chrome／type-to-confirm／單複數
        "知識檢索", "套用為正式設定", "欄位對應", "歷史紀錄", "資料匯入", "問答", // nav vs 功能標題 vs audit 動作詞彙
    ]);
    ...
    for (const f of srcHtml) {
        ...
        for (const m of html.matchAll(/label:\s*"([^"]*)"[^{}]*?i18nKey:\s*"([\w.]+)"/g)) recordKZ(m[2], m[1]);
        for (const m of html.matchAll(/i18nKey:\s*"([\w.]+)"[^{}]*?label:\s*"([^"]*)"/g)) recordKZ(m[1], m[2]);
    }
